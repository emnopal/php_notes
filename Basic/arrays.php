<?php
    ...
    // print array bisa juga pake var_dump
    ...
    $age = array("Peter"=>"35", "Ben"=>"37", "Joe"=>"43");
    echo PHP_EOL ."ambil semua keys milik array: " .PHP_EOL;
    print_r(array_keys($age)) .PHP_EOL;

    // ambil semua value milik array
    $age = array("Peter"=>"35", "Ben"=>"37", "Joe"=>"43");
    echo PHP_EOL ."ambil semua value milik array: " .PHP_EOL;
    print_r(array_values($age)) .PHP_EOL;

    // array_map equivalent map() di python
    $arr = [11.5,12.5,13.7,15.6,21,45,72];
    echo var_dump(array_map(fn($arr)=>$arr/10, $arr)) .PHP_EOL; // pake arrow func

    $useThisArrowFunc = fn($arr)=>$arr/2;
    echo var_dump(array_map($useThisArrowFunc, $arr)) .PHP_EOL; // pake arrow func

    echo var_dump(array_map(function(int|float $num):int|float{return $num*10;}, $arr)) .PHP_EOL; // pake lambda (anonymous) func

    $useThisAnonFunc = function(int|float $num):int|float{return $num*2;};
    echo var_dump(array_map($useThisAnonFunc, $arr)) .PHP_EOL; // pake lambda (anonymous) func, lain metode

    function useRealFunc(int|float $num): int|float {
        return $num+1;
    }

    echo var_dump(array_map("useRealFunc", $arr)) .PHP_EOL; // pake fungsi normal
